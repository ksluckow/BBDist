...
public class CFGToDotDistance {
   ...
   */
  public <N> DotGraph drawCFG(DirectedGraph<N> graph, Body body, Map<N, Integer> blockCounts) {
  ...
  canvas.drawEdge(namer.getName(node), namer.getName(succ));
    ...
    if (! isBrief) {
      formatNodeText(body, canvas, namer, blockCounts);
    }
  ...
  for (Iterator<ExceptionDest<N>> destsIt =
    ...
    setStyle(graph.getTails(), canvas, namer, 
       DotGraphConstants.NODE_STYLE_FILLED, tailAttr);
    if (! isBrief) {
      formatNodeText(graph.getBody(), canvas, namer, null);
    }
    return canvas;
  }
   ...
   */
  private <N> void formatNodeText(Body body, DotGraph canvas, DotNamer<N> namer, Map<N, Integer> blockCounts) {
   ...
   nodesIt.hasNext(); ) {
      ...
      String nodeLabel = "Distance: " + count + "\\n";
      if (node instanceof Block) {
        Block b = (Block)node;
        nodeLabel += "src line: " + b.getHead().getJavaSourceStartLineNumber() + "\\n";
      }
